...
const CustomNode = ({ data, id }) => {
  ...
  }, [preprocessingType, preprocessingOptions, data, id]);
  ...
  // Function to render LHS and RHS columns for reconciliation nodes
  const renderMatchingColumns = () => {
    return matchingRules.map((rule, index) => (
      <div key={index} className="matching-rule">
        <div className="lhs-column">
          <strong>LHS:</strong> {rule.lhsSelection.column} ({rule.lhsSelection.file_schema_id})
        </div>
        <div className="rhs-column">
          <strong>RHS:</strong> {rule.rhsSelection.column} ({rule.rhsSelection.file_schema_id})
        </div>
      </div>
    ));
  };
          ...
          {data.label === 'Reconciliation' && (
            <>
                ...
                <div key={index} className="matching-rule">
                  ...
                  <div className="dropdown-section">
                    ...
                    >
                      ...
                      {/* Add more matching methods if available */}
                    </select>
                  </div>
                  <div
                    className="delete_option node_btn violet"
                    onClick={() => removeMatchingRule(index)}
                    role="button"
                    tabIndex="0"
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') removeMatchingRule(index);
                    }}
                  >
              ...
              <div
                ...
                tabIndex="0"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') addMatchingRule();
                }}
              >
                <span>Add Rule</span>
